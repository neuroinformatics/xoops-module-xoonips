...
class XNPStimulusImportItemHandler extends XooNIpsImportItemHandler{
     ...
     */
    var $_stimulus_data = null;
    
    /**
     * flag of attachment file parsed
     */
    var $_stimulus_data_flag = false;
    ...
    function XNPStimulusImportItemHandler(){
    ...
    function xmlStartElementHandler( $parser , $name , $attribs ){
        ...
        case "ITEM/DETAIL/FILE":
            ...
            if( $this -> _file_type_attribute == 'stimulus_data' ){
                ...
                $this -> _stimulus_data =& $file_handler -> create();
                $this -> _stimulus_data -> setFilepath(
                    $this -> _attachment_dir . '/' . $attribs['FILE_NAME'] );
                $this -> _stimulus_data -> set( 
                    'original_file_name',
            ...
            if( $this -> _file_type_attribute == 'stimulus_data' ){
                ...
                if( !$file_handler -> insert( $this -> _stimulus_data ) ){
                    $this -> _import_item -> setErrors( 
                        ...
                        . $this -> _stimulus_data -> get(
                            'original_file_name' ) 
                        . $this -> _get_parser_error_at() );
                }
                $this -> _stimulus_data = $file_handler -> get(
                    $this -> _stimulus_data -> get( 'file_id' ) );
                $this -> _import_item -> setVar(
                    'stimulus_data', $this -> _stimulus_data );
    ...
    function onImportFinished( &$item, &$import_items ){
        if( 'xnpstimulusimportitem' != strtolower( get_class($item) ) ) return;
    ...
    function import(&$item){
        if ( $item -> getUpdateFlag() ) {
            ...
            if( $item -> hasStimulusData() ){
                $file_handler =& xoonips_getormhandler( 'xoonips', 'file' );
                $clonefile =& $file_handler -> fileClone( $stimulus_data );
                $clonefile -> setDirty();
