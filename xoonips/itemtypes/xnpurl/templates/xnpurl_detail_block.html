...
<!--//
function xnpurl_countup_link_<{$item_id}>() {
...
<table class="outer">
  ...
  <tr>
    ...
    <td class="<{cycle name="oddeven"}>">
      <a href="<{$xoonips_item.detail.url}>" target="_blank" onclick="return xnpurl_countup_link_<{$item_id}>();"><{$xoonips_item.detail.url}></a>
      <br />
     <{$smarty.const._MD_XNPURL_HITS}>:<{$xoonips_item.detail.url_count}>
    </td>
  </tr>
  <!-- file -->
  <tr>
    ...
    <td class="<{cycle name="oddeven"}>">
      ...
      <a href='<{$xoonips_item.detail.url}>'>
        <img src='<{$xoonips_item.detail.banner_image_url}>' alt='<{$xoonips_item.detail.url}>'/>
      </a>
      <{/if}>
    </td>
  </tr>
...
<input type="hidden" name="url_banner_fileFileID" value="<{$xoonips_item.detail.url_banner_file.file_id}>"/>
<{$index.hidden}>
</form>
<{/if}>
