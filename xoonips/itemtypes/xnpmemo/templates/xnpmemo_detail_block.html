...
<table class="outer">
  ...
  <tr>
    ...
    <td class="<{cycle name="oddeven"}>"><{if $xoonips_item.detail.item_link != ""}><a href="<{$xoonips_item.detail.item_link}>" target="_blank"><{$xoonips_item.detail.item_link}></a><{/if}></td>
  </tr>
  <!-- file -->
  <tr>
...
<{if $editable }>
<{$basic.hidden}>
<input type="hidden" name="item_link" value="<{$xoonips_item.detail.item_link}>"/>
<{$memo_file.hidden}>
<{$index.hidden}>
<{/if}>
</form>
