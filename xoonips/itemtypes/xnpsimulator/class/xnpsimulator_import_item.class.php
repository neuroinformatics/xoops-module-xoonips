...
class XNPSimulatorImportItemHandler extends XooNIpsImportItemHandler{
     ...
     */
    var $_simulator_data = null;
    
    /**
     * flag of attachment file parsed
     */
    var $_simulator_data_flag = false;
    ...
    function XNPSimulatorImportItemHandler(){
    ...
    function xmlStartElementHandler( $parser , $name , $attribs ){
        ...
        case "ITEM/DETAIL/FILE":
            ...
            if( $this -> _file_type_attribute == 'simulator_data' ){
                ...
                $this -> _simulator_data =& $file_handler -> create();
                $this -> _simulator_data -> setFilepath(
                    $this -> _attachment_dir . '/' . $attribs['FILE_NAME'] );
                $this -> _simulator_data -> set(
                    'original_file_name',
    ...
    function xmlEndElementHandler( $parser ,$name ){
        ...
        case "ITEM/DETAIL":
            ...
            if(count($this -> _import_item -> getVar('developer'))==0){
                $this -> _import_item -> setErrors(E_XOONIPS_TAG_NOT_FOUND, " no developer" 
                                                   . $this -> _get_parser_error_at( ) );
            }
            break;
        ...
        case "ITEM/DETAIL/CC_COMMERCIAL_USE": 
            $detail -> set(
                ...
                $unicode->decode_utf8(
                    $this -> _cdata ,xoonips_get_server_charset(),'h'), true );
            break;
        ...
        case "ITEM/DETAIL/FILE":
            ...
            if( $this -> _file_type_attribute == 'simulator_data' ){
                ...
                if( !$file_handler -> insert( $this -> _simulator_data ) ){
                    $this -> _import_item -> setErrors( 
                        E_XOONIPS_DB_QUERY,
                        "can't insert attachment file:"
                        . $this -> _simulator_data -> get(
                            'original_file_name' ) 
                        . $this -> _get_parser_error_at() );
                }
                $this -> _simulator_data = $file_handler -> get(
                    $this -> _simulator_data -> get( 'file_id' ) );
                $this -> _import_item -> setVar(
                    'simulator_data', $this -> _simulator_data );
    ...
    function getImportLog($import_item){
        ...
        $text .= "\ndetail.simulator_type " 
            . $detail -> get( "simulator_type" );
    ...
    function import(&$item){
        if ( $item -> getUpdateFlag() ) {
            ...
            if( $item -> hasSimulatorData() ){
                $file_handler =& xoonips_getormhandler( 'xoonips', 'file' );
                $clonefile =& $file_handler -> fileClone( $simulator_data );
                $clonefile -> setDirty();
