...
{
    function XooNIpsLogicImportCheckImport(){
        parent::XooNIpsLogic();
    }
    ...
    function execute(&$vars, &$response){
        ...
        if( $vars[2] ){ //import as new option is true
            ...
            foreach( array_keys( $vars[0] ) as $key ){
                if( $vars[0][$key] -> getDoiConflictFlag( ) ) continue;
                if( $vars[0][$key] -> getUpdateFlag( ) ) continue;
        ...
        $success = array(
            ...
            'private_item_number_limit_over' =>
                $this -> _is_private_item_number_limit_over( $vars[0],
                                                             $vars[1] ),
            'private_item_storage_limit_over'
                => $this -> _is_private_item_storage_limit_over( $vars[0],
                                                                 $vars[1] )
            );
    ...
    function _is_private_item_number_limit_over(&$import_items, $uid){
        $import_item_count = 0;
        foreach( $import_items as $item ){
            if( $item -> getImportAsNewFlag() ) $import_item_count++;
        }
    ...
    function _is_private_item_storage_limit_over(&$import_items, $uid){
    ...
    function _get_toal_file_size_of_item( $item_id ){
        $size = 0;
    ...
    function _get_total_file_size_of_update_items( $import_items ){
        $remove_filesize = 0;
        foreach( $import_items as $item ){
    ...
    function _get_total_file_size_of_import_items( $import_items ){
        $filesize = 0;
        foreach( $import_items as $item ){
    ...
    function _filesize_by_item_id( $iids ){
        if ( count( $iids ) == 0 ) {
            return 0;
        }
        $itemtypes = array(  );
        ...
        foreach( $itemtypes as $i ) {
            if( $i['item_type_id'] == ITID_INDEX ) continue;
            $modname = $i['name'];
            global $xoopsDB;
