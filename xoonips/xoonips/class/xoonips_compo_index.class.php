...
{
    function XooNIpsIndexCompoHandler(&$db) 
    {
     ...
     * @param integer $private_index_id integer value of private index(optional)
     * @param integer $fmt (optional)string format(see XoopsObject::getVar)
     * @return string array or false if failure
     */
    function getIndexPathNames($index_id, $private_index_id = false, $fmt='n'){
        $ret = array();
     ...
     */
    function getPathIndexes( $index_id ){
        $indexes = array();
        do{
            $xoonips_index = $this -> get( $index_id );
            if( !$xoonips_index ) return false;
     ...
     * @return integer index id of copy of $source_index_id or false if failure
     */
    function duplicateIndexStructure($target_index_id, $source_index_id)
    {
        $index_handler =& xoonips_getormhandler('xoonips', 'index');
