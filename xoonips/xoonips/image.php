...
if ( ! empty( $thumbnail ) ) {
  ...
  if ( $size > 0 ) {
    ...
    if ( preg_match( '/^([^\\/]*)\\/(.*)$/', $mime_type, $matches ) ) {
      if ( $matches[1] == 'audio' ) {
        $img_type = 'audio';
      } else if ( $matches[1] == 'image' ) {
        $img_type = 'image';
      } else if ( $matches[1] == 'video' ) {
        $img_type = 'video';
      } else if ( $matches[1] == 'text' ) {
        $img_type = 'text';
      } else if ( $matches[1] == 'application' ) {
        $text_types = array(
          'pdf',
          'xml',
          'msword',
        ...
        if ( in_array( $matches[2], $text_types ) ) {
          $img_type = 'text';
        } else if ( in_array( $matches[2], $audio_types ) ) {
          $img_type = 'audio';
        } else if ( in_array( $matches[2], $image_types ) ) {
          $img_type = 'image';
        } else {
